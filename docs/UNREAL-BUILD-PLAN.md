...
**Status:** `Manhattan` and `Band` are generated, wired, compiled and saved,
and `BP_SelfTest` runs and passes on all eight assertions. `InBounds`,
`Approach` and `TileToWorld` are generated and waiting on the editor; the
self-test is at 66 nodes and 15 assertions for when they land.
