...
**Status:** the round loop is closed. Eleven graphs are generated, wired,
compiled and saved -- `Manhattan`, `Band`, `InBounds`, `Approach`,
`TileToWorld`, `StepToward`, `Predict`, `RexAct`, `SyncActors`, `BeginPlay`
and `OnPlayerMove` -- plus nine key events that call into it.
`BP_SelfTest` runs and passes 28 assertions, including the veto invariant and
the refused move.

What that adds up to: pressing D advances a round, Rex predicts, vetoes its own
prediction when following it would take it further from the dog, and the three
actors move to the tiles the logic believes they are on. Pressing W at the
north fence prints "The fence is there. Nothing on that side." and costs
nothing.

What is still missing from the acceptance sentence: **the robot never speaks**
(`SpeakRead`, `FiringCategories`, `DirFreq`), **nothing ends** (`CheckEnd`),
and **there is no HUD**. Those are the next three, in that order.

The node classes for all of it were captured out of the editor rather than
guessed -- `K2Node_VariableGet`, `K2Node_VariableSet`, `Array_Length`,
`Array_Add`, `K2Node_GetArrayItem`, `K2Node_IfThenElse`, `K2Node_InputKey`,
`GetAllActorsOfClass` and `K2_SetActorLocation`. See
`docs/UNREAL-NODE-TEMPLATES.md` for how, and for the two that are still
missing: `K2Node_GetDataTableRow` for the reads, and whatever the Enhanced
Input subsystem calls serialise as if IMC_Fight is ever swapped in for the raw
keys.

Variable references resolve by name alone, so `MemberGuid` is optional --
which matters, because Blueprint's `new_variables` array is not exposed to
Python and the guids can only be read off a node that already exists.
