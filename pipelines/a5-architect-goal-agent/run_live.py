...
def run(label, cmd):
    ...
    say("=" * 70)
    # Stream the child line by line rather than capturing it. A captured
    # subprocess prints nothing until it exits, which reads as a hang.
    p = subprocess.Popen([sys.executable, "-u"] + cmd[1:], cwd=ROOT,
