...
def main():
    say("Rex Machina, Assignment 8 live run")
    ...
    rc = run("STEP 1 of 4, the nine-turn session", ["scripted.py", "--live"])
    if rc:
        say("")
    ...
    run("STEP 2 of 4, the reactive A/B", ["ab_test.py", "--live"])
    run("STEP 3 of 4, the engine rules", ["guards.py"])
    run("STEP 4 of 4, the tests", ["tests/test_dm.py"])
